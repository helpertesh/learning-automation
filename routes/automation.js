...
router.get('/weak-topics', (_req, res) => {
...
router.get('/quiz-progress', (_req, res) => {
  const stats = {
    total_quizzes: db.prepare('SELECT COUNT(*) AS c FROM quizzes').get().c,
    total_attempts: db.prepare('SELECT COUNT(*) AS c FROM quiz_attempts WHERE completed_at IS NOT NULL').get().c,
    ...
    `).get().avg || 0,
    summaries_created: db.prepare('SELECT COUNT(*) AS c FROM note_summaries').get().c,
  };
