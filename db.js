...
function migrateQuizQuestions() {
  const cols = db.exec('PRAGMA table_info(quiz_questions)');
  const names = cols[0]?.values?.map((r) => r[1]) || [];
  if (!names.includes('source_paper')) {
    exec('ALTER TABLE quiz_questions ADD COLUMN source_paper TEXT');
  }
  if (!names.includes('note_refs_json')) {
    exec('ALTER TABLE quiz_questions ADD COLUMN note_refs_json TEXT');
  }
}

function migrateStudySessions() {
  const cols = db.exec("PRAGMA table_info(study_sessions)");
  const names = cols[0]?.values?.map((r) => r[1]) || [];
  if (!names.includes('topic_name')) {
    exec('ALTER TABLE study_sessions ADD COLUMN topic_name TEXT');
  }
  if (!names.includes('session_type')) {
    exec("ALTER TABLE study_sessions ADD COLUMN session_type TEXT DEFAULT 'manual'");
  }
}

module.exports = { init, prepare, exec };
